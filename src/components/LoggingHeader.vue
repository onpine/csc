<template>
  <div class="header">
    ...
    <ul>
      ...
      <li class="userid">
        <a @click="changego('/LoggingStatus/PersonalCenter')">{{user.username}}</a>
        <ul class="down">
          <li>
            <a href="/#/LoggingStatus/PersonalCenter" target="_blank">个人中心</a>
          </li>
          <li>
            <a href>*上传资源</a>
          </li>
          <li>
            <a href="/#/LoggingStatus/BlogEditor/0">创作</a>
          </li>
...
export default {
  ...
  methods: {
    ...
    exit() {
      let that = this;
      let accessToken = this.$store.getters.getsessionId;
      if (accessToken == "") {
